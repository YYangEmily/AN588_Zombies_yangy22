---
title: "yangy22__FinalHomeworkCode_03"
author: "Emily Yang"
date: "`r Sys.Date()`"
...
p + ggtitle("Gender vs Height") + xlab("Gender") + ylab("Height") # Add axis labels
q <- ggplot(d, aes(x=gender, y=weight, color=gender)) + geom_boxplot()
q + ggtitle("Gender vs Weight") + xlab("Gender") + ylab("Weight")
r <- ggplot(d, aes(x=gender, y=zombies_killed, color=gender)) + geom_boxplot()
r + ggtitle("Gender vs Zombies Killed") + xlab("Gender") + ylab("Zombies Killed")
s <- ggplot(d, aes(x=gender, y=years_of_education, color=gender)) + geom_boxplot()
s + ggtitle("Gender vs Years of Education") + xlab("Gender") + ylab("Years of Education")
...
sd<- sd(h)
sem <- sd(h)/sqrt(length(h))
# Winging these confidence intervals
lower <- m - qnorm(1 - 0.05/2) * sem  
upper <- m + qnorm(1 - 0.05/2) * sem
ci <- c(lower, upper)
Zkill <-c(m, sd, sem, ci)
...
New
```
<br> The distributions look normally distributed even the ones that came from Poisson distributions because we moved the center to be the mean of 30 samples. The means are similar to before but the standard deviation dropped because we averaged between samples. <br>

<br>Reflections added 10/19/23: I had challenges working with the gender and comparing it with the different variables in Question 2. I wanted to separate out the gender groups and then make the graphs and I couldn't get it to subset. I ended up finding that making it one graph automatically separates out by gender. Which should have been obvious. Another challenge I had was that I didn't know how to find the CI of the two non-normal distributions. I used the regular CI interval formula for normal distributions. Third challenge was for question 6 I didn't add the sample from question 5 I just picked 100 new samples. I assumed that since they were picked the same way it shouldn't change things very much and I didn't want to try adding data across chunks. I didn't run into any other challenges. <br>
